...
export class HasMany<R extends typeof Model> extends Relation<R> {
  async get(): Promise<InstanceType<R>[]> {
    const r = this.related as unknown as {
      db: () => any;
      table: Record<string, unknown>;
      hydrate: (row: Record<string, unknown>) => InstanceType<R>;
    };
    ...
    const rows = (await r
      ...
      .select()
      .from(r.table)
      .where(eq(r.table[this.foreignKey] as never, localValue as never))) as Record<string, unknown>[];
...
export class HasOne<R extends typeof Model> extends Relation<R> {
  async get(): Promise<InstanceType<R> | null> {
    ...
    const rows = (await r
      ...
      .select()
      .from(r.table)
      .where(eq(r.table[this.foreignKey] as never, localValue as never))
...
export class BelongsTo<R extends typeof Model> extends Relation<R> {
  async get(): Promise<InstanceType<R> | null> {
    ...
    const rows = (await r
      ...
      .select()
      .from(r.table)
      .where(eq(r.table[this.localKey] as never, fkValue as never))
      .limit(1)) as Record<string, unknown>[];
    return rows.length ? r.hydrate(rows[0]!) : null;
  }
}
